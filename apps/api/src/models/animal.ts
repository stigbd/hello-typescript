...
export const AnimalSchema = z.discriminatedUnion("type", [CatSchema, DogSchema]).openapi("Animal", {
  description: "An animal (cat or dog)",
});

// Input schemas (without the type discriminator) for creating animals
export const CreateCatSchema = CatSchema.omit({ type: true }).openapi("CreateCat", {
  description: "Data required to create a cat",
});

export const CreateDogSchema = DogSchema.omit({ type: true }).openapi("CreateDog", {
 ...
 *
 * Note: The inferred types are compatible with the types from @hello-typescript/types
 ...
 *
 * The Zod-inferred types (Cat, Dog, Animal) are compatible with the types
 * from @hello-typescript/types. The Zod schemas provide runtime validation while
 * the types package provides compile-time type checking across the monorepo.
 *
